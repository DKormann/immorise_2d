...
class Model(nn.Module):
  def __init__(self, model_scale=1., n_toks=100):
    ...
    self.pt_emb = nn.Embedding(n_toks, self.residual_depth//2, scale_grad_by_freq=True)
    self.blocks = nn.ModuleList([LatentAttentionBlock(self.residual_depth) for _ in range(num_blocks)])
    self.blocks2 = nn.ModuleList([LatentCrossAttentionBlock(self.residual_depth) for _ in range(num_blocks)])
  ...
  def forward(self, q:torch.Tensor, pts:torch.Tensor):
    ...
    kv = kv.view(kv.shape[0], -1, self.residual_depth) # B, 100 , D
