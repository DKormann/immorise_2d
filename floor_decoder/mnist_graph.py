...
layers = [[init(D, D*3) , init(D, D*4), init(D*4,(10 if i == layers-1  else D))] for i in range(layers)]

def forward(x):
  x = x.unsqueeze(-1) * torch.exp(torch.arange(0, D, 4).float() * -(7. / D))
  x = torch.cat([torch.sin(x), torch.cos(x)], -1).view(*x.shape[:-2],-1)
  for qkv, l1, out in layers:
    q,k,v = (x@qkv).chunk(3, dim=-1)
    attention_result = ((q @ k.transpose(-2, -1)) / (q.shape[-1])**.5).softmax(-1) @ v
    x = (((x + attention_result) @l1).relu()) @ out
  return x.mean(1)

opt = torch.optim.Adam(sum(layers, []), lr=1e-6)
try: 
...
labels[k]
# %%
plt.scatter(*points[k].T.cpu().numpy())
